...
namespace
{
    const char s_version[] = "JSProxy Session Decrypt PoC version 1.0.0";

    bool parseCommandLine(int p_argCount, const char* p_argArray[],
                          std::string& p_file, std::string& p_masterKey)
    {
        boost::program_options::options_description description("options");
        description.add_options()
        ("help", "A list of command line options")
        ("version", "Display version information")
        ("masterkey,m", boost::program_options::value<std::string>(),
         "A 128 bit master key to compute the receive and send RC4 with.")
        ...
        if (argv_map.count("file") && argv_map.count("masterkey")) {
            p_file.assign(argv_map["file"].as<std::string>());
            p_masterKey.assign(argv_map["masterkey"].as<std::string>());
            if (p_masterKey.size() != 32) {
                std::cerr << "The master key must be 32 bytes." << std::endl;
                return false;
            }
            std::string masterKey;
            for (std::size_t i = 0; i < p_masterKey.size(); i += 2) {
                std::string temp;
                temp.push_back(p_masterKey[i]);
                temp.push_back(p_masterKey[i+1]);
                masterKey.push_back(strtol(temp.data(), NULL, 16));
            }
            p_masterKey.assign(masterKey);
            return true;
        } else {
            std::cout << description << std::endl;
        }
...
int main(int p_argc, const char** p_argv)
{
    std::string fileName;
    std::string masterKey;
    if (!parseCommandLine(p_argc, p_argv, fileName, masterKey)) {
        return EXIT_FAILURE;
    }
    ...
    SessionParser parser(masterKey);
